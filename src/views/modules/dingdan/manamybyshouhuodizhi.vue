    ...
    <template v-if="showFlag">
      ...
      <el-form :inline="true" :model="searchForm" class="dy-list-search-el-form">
        <el-row class="dy-list-search-el-form-row">
          ...
          <div class="dy-list-search-el-form-row-div">
            <label class="dy-list-search-el-form-row-div-label">订单金额</label>
            <el-input style="width: auto;" suffix-icon="el-icon-search" v-model="searchForm.jine" placeholder="请输入订单金额" clearable></el-input>
          </div>
          <div class="dy-list-search-el-form-row-div">
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: "dingdan/manamybyshouhuodizhi",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    getShouhuodizhiList() {
      this.$http({
        url: "shouhuodizhi/search?usersid="+this.$storage.get('usersid'),
        method: "get",
        params: null
      }).then(({ data }) => {
        if (data && data.code === 0) {
