      ...
      <el-form :inline="true" :model="searchForm" class="dy-list-search-el-form">
        ...
        <el-row class="dy-list-search-el-form-row-2">
          ...
          <el-button type="info" icon="el-icon-download" @click="exportExcel" v-show="false">Excel 导出</el-button>
            <!--上传Excel文件批量导入-->
            <excel-file-upload action="lunbo/excelImport" :limit="1" :multiple="true" style="margin-left: 20px;" v-show="false"></excel-file-upload>
        </el-row>
      </el-form>
      <!--数据列表区域-->
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-show="true" class="dy-list-detail-button" type="success" icon="el-icon-info" size="mini" @click="lunboDetailHandler(scope.row.id)">详情</el-button>
            <el-button v-show="true" class="dy-list-update-button" type="primary" icon="el-icon-edit" size="mini" @click="lunboSetHandler(scope.row.id)">修改</el-button>
            <el-button v-show="true" class="dy-list-delete-button" type="danger" icon="el-icon-delete" size="mini" @click="lunboDeleteHandler(scope.row.id)">删除</el-button>
...
export default {
  ...
  methods: {
    ...
    //excel导出
    exportExcel() {
      let url = this.$base.url + "/lunbo/exportExcel";
      window.open(url);
    },
