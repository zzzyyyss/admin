    ...
    <template v-if="showFlag">
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
              <el-button class="dy-list-shehe-button" type="primary" icon="el-icon-edit-outline" size="mini" @click="shangpinSet(scope.row.id,scope.row.shenhe == 'no' ? '是否特价' : '取消特价','shenhe',scope.row.shenhe == 'no' ? 'yes' : 'no')">{{scope.row.shenhe == 'no' ? '是否特价' : '取消特价' }}</el-button>
