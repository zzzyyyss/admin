<template>
  <div class="dy-admin-router-view-div">
      ...
      <el-form :inline="true" :model="searchForm" class="dy-list-search-el-form">
        <el-row class="dy-list-search-el-form-row">
          <div class="dy-list-search-el-form-row-div">
            ...
            <el-input style="width: auto;" suffix-icon="el-icon-search" v-model="searchForm.biaoti" placeholder="请输入评语" clearable></el-input>
          </div>
          <div class="dy-list-search-el-form-row-div">
            <label class="dy-list-search-el-form-row-div-label">分数（范围1-5）</label>
            <el-input style="width: auto;" suffix-icon="el-icon-search" v-model="searchForm.fenshu" placeholder="请输入分数（范围1-5）" clearable></el-input>
          </div>
          <div class="dy-list-search-el-form-row-div">
            <label class="dy-list-search-el-form-row-div-label">配送</label>
            <el-select v-model="searchForm.peisongid" placeholder="请选择配送" clearable>
              <el-option label="全部" value=""></el-option>
              <el-option v-for="(item, index) in peisongOptions" v-bind:key="item.id" :label="item.xingming" :value="item.id">
              </el-option>
            </el-select>
          </div>
          <div class="dy-list-search-el-form-row-div">
        ...
        <el-row class="dy-list-search-el-form-row-2">
          <el-button class="dy-list-batchdelete-button" :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="pingjiaDeleteHandler()">批量删除</el-button>
          <!--将数据导出Excel文件-->
          <el-button type="info" icon="el-icon-download" @click="exportExcel" v-show="false">Excel 导出</el-button>
            <!--上传Excel文件批量导入-->
            <excel-file-upload action="pingjia/excelImport" :limit="1" :multiple="true" style="margin-left: 20px;" v-show="false"></excel-file-upload>
        </el-row>
      </el-form>
      <!--数据列表区域-->
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column label="序号" v-if="true" type="index" width="70" />
          ...
          <el-table-column label="配送">
            <template slot-scope="scope">
              <div v-if="scope.row.peisong && scope.row.peisong.xingming">{{ scope.row.peisong.xingming }}</div>
              <div v-else>--</div>
            </template>
          </el-table-column>
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-show="true" class="dy-list-detail-button" type="success" icon="el-icon-info" size="mini" @click="pingjiaDetailHandler(scope.row.id)">详情</el-button>
            <el-button v-show="true" class="dy-list-update-button" type="primary" icon="el-icon-edit" size="mini" @click="pingjiaSetHandler(scope.row.id)">修改</el-button>
            <el-button v-show="true" class="dy-list-delete-button" type="danger" icon="el-icon-delete" size="mini" @click="pingjiaDeleteHandler(scope.row.id)">删除</el-button>
    ...
    <!-- 评价修改-->
    <pingjia-set v-if="pingjiaSetFlag" :parent="this" ref="pingjiaSet"></pingjia-set>
    <!-- 评价详情-->
    <pingjia-detail v-if="pingjiaDetailFlag" :parent="this" ref="pingjiaDetail"></pingjia-detail>

  </div>
</template>
<script>
import PingjiaSet from "./pingjia-set"
import PingjiaDetail from "./pingjia-detail"
...
export default {
  data() {
    return {
      ...
      dataListSelections: [],
      showFlag: true,
      pingjiaSetFlag: false,
      pingjiaDetailFlag: false,
  ...
  methods: {
    ...
    getDataList() {
      ...
      if (this.searchForm.peisongid != '' && this.searchForm.peisongid != undefined) {
        params['peisongid'] = this.searchForm.peisongid
      }
    ...
    //跳转到评价详情页面
    pingjiaDetailHandler(id) {
      this.showFlag = false;
      this.pingjiaDetailFlag = true;
      this.$nextTick(() => {
        this.$refs.pingjiaDetail.init(id);
      });

    },

    //跳转到评价修改页面
    pingjiaSetHandler(id) {
      this.showFlag = false;
      this.pingjiaSetFlag = true;
      this.$nextTick(() => {
        this.$refs.pingjiaSet.init(id);
      });
    },

    //更新评价信息操作方法
    pingjiaSet(tid,tname,property,value) {
      let params = {};
      params['id'] = tid;
    ...
    //excel导出
    exportExcel() {
      let url = this.$base.url + "/pingjia/exportExcel";
      window.open(url);
    },
