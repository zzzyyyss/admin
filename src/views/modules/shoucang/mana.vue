<template>
  <div class="dy-admin-router-view-div">
    <template v-if="showFlag">
      ...
      <el-form :inline="true" :model="searchForm" class="dy-list-search-el-form">
        ...
        <el-row class="dy-list-search-el-form-row-2">
          <el-button class="dy-list-batchdelete-button" :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="shoucangDeleteHandler()">批量删除</el-button>
          <!--将数据导出Excel文件-->
          <el-button type="info" icon="el-icon-download" @click="exportExcel" v-show="false">Excel 导出</el-button>
            <!--上传Excel文件批量导入-->
            <excel-file-upload action="shoucang/excelImport" :limit="1" :multiple="true" style="margin-left: 20px;" v-show="false"></excel-file-upload>
        </el-row>
      </el-form>
      <!--数据列表区域-->
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-show="true" class="dy-list-detail-button" type="success" icon="el-icon-info" size="mini" @click="shoucangDetailHandler(scope.row.id)">详情</el-button>
            <el-button v-show="true" class="dy-list-update-button" type="primary" icon="el-icon-edit" size="mini" @click="shoucangSetHandler(scope.row.id)">修改</el-button>
            <el-button v-show="true" class="dy-list-delete-button" type="danger" icon="el-icon-delete" size="mini" @click="shoucangDeleteHandler(scope.row.id)">删除</el-button>
    ...
    <!-- 商品收藏修改-->
    <shoucang-set v-if="shoucangSetFlag" :parent="this" ref="shoucangSet"></shoucang-set>
    <!-- 商品收藏详情-->
    <shoucang-detail v-if="shoucangDetailFlag" :parent="this" ref="shoucangDetail"></shoucang-detail>

  </div>
</template>
<script>
import ShoucangSet from "./shoucang-set"
import ShoucangDetail from "./shoucang-detail"
...
export default {
  data() {
    return {
      ...
      dataListSelections: [],
      showFlag: true,
      shoucangSetFlag: false,
      shoucangDetailFlag: false,
  ...
  methods: {
    ...
    //跳转到商品收藏详情页面
    shoucangDetailHandler(id) {
      this.showFlag = false;
      this.shoucangDetailFlag = true;
      this.$nextTick(() => {
        this.$refs.shoucangDetail.init(id);
      });

    },

    //跳转到商品收藏修改页面
    shoucangSetHandler(id) {
      this.showFlag = false;
      this.shoucangSetFlag = true;
      this.$nextTick(() => {
        this.$refs.shoucangSet.init(id);
      });
    },

    //更新商品收藏信息操作方法
    shoucangSet(tid,tname,property,value) {
      let params = {};
      params['id'] = tid;
    ...
    //excel导出
    exportExcel() {
      let url = this.$base.url + "/shoucang/exportExcel";
      window.open(url);
    },
