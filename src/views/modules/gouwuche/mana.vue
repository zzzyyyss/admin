<template>
  <div class="dy-admin-router-view-div">
    <template v-if="showFlag">
      ...
      <el-form :inline="true" :model="searchForm" class="dy-list-search-el-form">
        ...
        <el-row class="dy-list-search-el-form-row-2">
          <el-button class="dy-list-batchdelete-button" :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="gouwucheDeleteHandler()">批量删除</el-button>
          <!--将数据导出Excel文件-->
          <el-button type="info" icon="el-icon-download" @click="exportExcel" v-show="false">Excel 导出</el-button>
            <!--上传Excel文件批量导入-->
            <excel-file-upload action="gouwuche/excelImport" :limit="1" :multiple="true" style="margin-left: 20px;" v-show="false"></excel-file-upload>
        </el-row>
      </el-form>
      <!--数据列表区域-->
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-show="true" class="dy-list-detail-button" type="success" icon="el-icon-info" size="mini" @click="gouwucheDetailHandler(scope.row.id)">详情</el-button>
            <el-button v-show="true" class="dy-list-update-button" type="primary" icon="el-icon-edit" size="mini" @click="gouwucheSetHandler(scope.row.id)">修改</el-button>
            <el-button v-show="true" class="dy-list-delete-button" type="danger" icon="el-icon-delete" size="mini" @click="gouwucheDeleteHandler(scope.row.id)">删除</el-button>
    ...
    <!-- 购物车修改-->
    <gouwuche-set v-if="gouwucheSetFlag" :parent="this" ref="gouwucheSet"></gouwuche-set>
    <!-- 购物车详情-->
    <gouwuche-detail v-if="gouwucheDetailFlag" :parent="this" ref="gouwucheDetail"></gouwuche-detail>

  </div>
</template>
<script>
import GouwucheSet from "./gouwuche-set"
import GouwucheDetail from "./gouwuche-detail"
...
export default {
  data() {
    return {
      ...
      dataListSelections: [],
      showFlag: true,
      gouwucheSetFlag: false,
      gouwucheDetailFlag: false,
  ...
  methods: {
    ...
    //跳转到购物车详情页面
    gouwucheDetailHandler(id) {
      this.showFlag = false;
      this.gouwucheDetailFlag = true;
      this.$nextTick(() => {
        this.$refs.gouwucheDetail.init(id);
      });

    },

    //跳转到购物车修改页面
    gouwucheSetHandler(id) {
      this.showFlag = false;
      this.gouwucheSetFlag = true;
      this.$nextTick(() => {
        this.$refs.gouwucheSet.init(id);
      });
    },

    //更新购物车信息操作方法
    gouwucheSet(tid,tname,property,value) {
      let params = {};
      params['id'] = tid;
    ...
    //excel导出
    exportExcel() {
      let url = this.$base.url + "/gouwuche/exportExcel";
      window.open(url);
    },
