<template>
  <div v-if="this.parent.fukuanfangshiSetFlag">
    <el-form class="dy-add-set-detail-el-form" ref="ruleForm" :model="ruleForm" :rules="rules" label-width="110px">
      <template>
        <el-form-item class="dy-add-set-detail-el-form-item" label="支付方式" prop="mingcheng">
          <el-input v-model="ruleForm.mingcheng" placeholder="请输入支付方式" clearable
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        mingcheng: [{ required: true, message: '支付方式不能为空', trigger: 'blur' },],
  ...
  methods: {
    ...
    back() {
      if (this.parent) {
        ...
        this.$router.replace('/fukuanfangshimana')
      }
    },
