<template>
  <div v-if="this.parent.gonggaoSetFlag">
    <el-form class="dy-add-set-detail-el-form" ref="ruleForm" :model="ruleForm" :rules="rules" label-width="110px">
      <template>
        ...
        <el-form-item class="dy-add-set-detail-el-form-item" label="图片" prop="tupian">
          <file-upload tip="点击上传图片" action="common/uploadfile" :limit="1" :multiple="true"
            @change="tupianUploadChange" :fileUrls="ruleForm.tupian ? getFileSrc(ruleForm.tupian) : ''"></file-upload>
        </el-form-item>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        neirong: [],
        tupian: [{ required: true, message: '图片不能为空', trigger: 'blur' },],
  ...
  methods: {
    ...
    back() {
      if (this.parent) {
        ...
        this.parent.gonggaoSetFlag = false;
        this.parent.gonggaoDetailFlag = false;
      } else {
        this.$router.replace('/gonggaomana')
      }
    },
