    ...
    <template v-if="showFlag">
      ...
      <el-form :inline="true" :model="searchForm" class="dy-list-search-el-form">
        ...
        <el-row class="dy-list-search-el-form-row-2">
          ...
          <el-button type="info" icon="el-icon-download" @click="exportExcel" v-show="false">Excel 导出</el-button>
            <!--上传Excel文件批量导入-->
            <excel-file-upload action="peisong/excelImport" :limit="1" :multiple="true" style="margin-left: 20px;" v-show="false"></excel-file-upload>
        </el-row>
      </el-form>
      <!--数据列表区域-->
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
              <el-button v-show="scope.row.songshenhe == 'no'" class="dy-list-shehe-button" type="primary" icon="el-icon-edit-outline" size="mini" @click="peisongSet(scope.row.id,'是否接单','songshenhe', 'yes')">是否接单</el-button>
              <el-button v-show="scope.row.songshenhe == 'yes'" class="dy-list-shehe-button" type="primary" icon="el-icon-edit-outline" size="mini" @click="peisongSet(scope.row.id,'取消是否接单','songshenhe', 'no')">取消是否接单</el-button>

            <el-button v-show="true" class="dy-list-detail-button" type="success" icon="el-icon-info" size="mini" @click="peisongDetailHandler(scope.row.id)">详情</el-button>
            <el-button v-show="true" class="dy-list-update-button" type="primary" icon="el-icon-edit" size="mini" @click="peisongSetHandler(scope.row.id)">修改</el-button>
            <el-button v-show="true" class="dy-list-delete-button" type="danger" icon="el-icon-delete" size="mini" @click="peisongDeleteHandler(scope.row.id)">删除</el-button>
...
<script>
import PeisongSet from "./peisong-set"
import PeisongDetail from "./peisong-detail"
import PeisongPingjiaChakan from './peisong-pingjia-chakan'
import PeisongPingjiaAdd from './peisong-pingjia-add'
...
export default {
  ...
  methods: {
    ...
    //excel导出
    exportExcel() {
      let url = this.$base.url + "/peisong/exportExcel";
      window.open(url);
    },


    //跳转到配送评价添加页面
    pingjiaAddHandler(id, xingming) {
      this.showFlag = false;
