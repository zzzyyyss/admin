<template>
  <div>
    <template v-if="this.parent.usersShangpinChakanFlag && showFlag">
      <!--搜索表单区域-->
      ...
      @selection-change="selectionChangeHandler" :border="true">
        ...
        <el-table-column width="350" label="操作">
          <template slot-scope="scope">
              <!-- <el-button class="dy-list-shehe-button" type="primary" icon="el-icon-edit-outline" size="mini" @click="shangpinSet(scope.row.id,scope.row.shenhe == 'no' ? '是否特价' : '取消是否特价','shenhe',scope.row.shenhe == 'no' ? 'yes' : 'no')">{{scope.row.shenhe == 'no' ? '是否特价' : '取消是否特价' }}</el-button> -->


            <el-button class="dy-list-detail-button" type="success" icon="el-icon-info" size="mini" @click="shangpinDetailHandler(scope.row.id)">详情</el-button>
            <!-- <el-button class="dy-list-update-button" type="primary" icon="el-icon-edit" size="mini" @click="shangpinSetHandler(scope.row.id)">修改</el-button> -->
            <!-- <el-button class="dy-list-delete-button" type="danger" icon="el-icon-delete" size="mini" @click="shangpinDeleteHandler(scope.row.id)">删除</el-button> -->
...
<script>
import ShangpinSet from "../shangpin/shangpin-set"
import ShangpinDetail from "../shangpin/shangpin-detail"
import ShangpinChanpinpingjiaChakan from '../shangpin/shangpin-chanpinpingjia-chakan'
import ShangpinChanpinpingjiaAdd from '../shangpin/shangpin-chanpinpingjia-add'
import ShangpinDingdanxiangChakan from '../shangpin/shangpin-dingdanxiang-chakan'
import ShangpinDingdanxiangAdd from '../shangpin/shangpin-dingdanxiang-add'
import ShangpinGouwucheChakan from '../shangpin/shangpin-gouwuche-chakan'
import ShangpinGouwucheAdd from '../shangpin/shangpin-gouwuche-add'
import ShangpinShoucangChakan from '../shangpin/shangpin-shoucang-chakan'
import ShangpinShoucangAdd from '../shangpin/shangpin-shoucang-add'
  ...
  methods: {
    ...
    back() {
      if (this.parent) {
        ...
        this.parent.usersShangpinChakanFlag = false;
      } else {
        this.$router.push('/');
      }
    },
